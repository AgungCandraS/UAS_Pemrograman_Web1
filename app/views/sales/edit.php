<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 style="color: var(--text-primary); font-weight: 700; margin: 0;">
                <i class="fas fa-edit me-2" style="color: var(--primary);"></i>Edit Penjualan
            </h4>
            <p style="color: var(--text-secondary); margin: 0; margin-top: 0.5rem;">
                Edit transaksi penjualan #<?= htmlspecialchars($sale['sale_number']) ?>
            </p>
        </div>
    ...
    <form method="POST" action="<?= base_url('sales/update/' . $sale['id']) ?>" id="saleForm">
        <div class="row">
            <!-- Left Column -->
            <div class="col-lg-8">
                <!-- Sale Type & Platform -->
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="sale_type" id="saleType" class="form-select" required style="background: var(--surface-2); border: 1px solid var(--border-color); color: var(--text-primary);">
                                <option value="offline" <?= $sale['sale_type'] === 'offline' ? 'selected' : '' ?>>Offline (Tanpa Admin Fee)</option>
                                <option value="online" <?= $sale['sale_type'] === 'online' ? 'selected' : '' ?>>Online (Dengan Admin Fee)</option>
                            </select>
                        </div>
                        <div class="col-md-6" id="platformWrapper" style="display: <?= $sale['sale_type'] === 'online' ? 'block' : 'none' ?>;">
                            <label class="form-label" style="color: var(--text-secondary); font-weight: 500;">
                            ...
                            <select name="admin_fee_setting_id" id="platformSelect" class="form-select" style="background: var(--surface-2); border: 1px solid var(--border-color); color: var(--text-primary);">
                                ...
                                <?php foreach ($adminFeeSettings as $platform): ?>
                                    <option value="<?= $platform['id'] ?>" 
                                            ...
                                            <?= $sale['admin_fee_setting_id'] == $platform['id'] ? 'selected' : '' ?>>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="customer_address" class="form-control" rows="2" 
                                      ...
                                      style="background: var(--surface-2); border: 1px solid var(--border-color); color: var(--text-primary);"><?= htmlspecialchars($sale['customer_address'] ?? '') ?></textarea>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody id="productRows">
                                <?php foreach ($items as $index => $item): ?>
                                    <tr class="product-row" data-row-index="<?= $index ?>">
                                        <td>
                                            <select class="form-select form-select-sm product-select" required onchange="updateProductInfo(this)" style="background: var(--surface-2); border: 1px solid var(--border-color); color: var(--text-primary);" name="items[<?= $index ?>][product_id]">
                                            ...
                                            <input type="hidden" class="product-name" name="items[<?= $index ?>][product_name]" value="<?= htmlspecialchars($item['product_name']) ?>">
                                            <input type="hidden" class="product-sku" name="items[<?= $index ?>][product_sku]" value="<?= htmlspecialchars($item['product_sku']) ?>">
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <input type="number" class="form-control form-control-sm quantity-input" min="1" required onchange="calculateRowTotal(this)" 
                                                   value="<?= $item['quantity'] ?>"
            ...
            <div class="col-lg-4">
                <div style="background: var(--surface-1); border: 1px solid var(--border-color); border-radius: 14px; padding: 1.5rem; position: sticky; top: 20px;">
                    ...
                    <div id="adminFeeSection" style="display: <?= $sale['sale_type'] === 'online' ? 'block' : 'none' ?>;">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control" rows="3" 
                                  ...
                                  style="background: var(--surface-2); border: 1px solid var(--border-color); color: var(--text-primary);"><?= htmlspecialchars($sale['notes'] ?? '') ?></textarea>
                    </div>

                    <!-- Submit Button -->
                    <button type="submit" class="btn btn-primary w-100" style="padding: 0.75rem;">
                        <i class="fas fa-save me-2"></i>Update Penjualan
                    </button>
                </div>
            </div>
        </div>
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize platform info if online
